#ifndef QUIZ_H
#define QUIZ_H

#include "GradedItem.h"

class Quiz : public GradedItem {
    string name;
    float pointsEarned;
    float pointsPossible;
public:
    Quiz(const string& name, float earned, float possible)
        : name(name), pointsEarned(earned), pointsPossible(possible) {}
    string getName() const override { return name; }
    string getCategory() const override { return "Quiz"; }
    float getPointsEarned() const override { return pointsEarned; }
    float getPointsPossible() const override { return pointsPossible; }
    float getPercentage() const override {
        return (pointsPossible > 0) ? (pointsEarned / pointsPossible) * 100 : 0;
    }

    void setPointsEarned(float earned) override { pointsEarned = earned; }
    void setPointsPossible(float possible) override { pointsPossible = possible; }
};

#endif
